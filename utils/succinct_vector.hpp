...
namespace utils
{
  ...
  public:
	...
	throw std::runtime_error("not a succinct vector...");
      ...
      // setup cache...
      const size_type select0_size = std::max(size_type(utils::bithack::next_largest_power2((size() - __rank_high.back()) >> 5)),
					      size_type(1024 * 32));
      const size_type select1_size = std::max(size_type(utils::bithack::next_largest_power2(__rank_high.back() >> 5)),
					      size_type(1024 * 32));
